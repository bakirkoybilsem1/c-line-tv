...
export default function StudentUpload() {
  ...
  async function handleSubmit() {
    if (!word || !photo || !video) {
      setError('Lütfen kelime, fotoğraf ve video ekleyin.')
      return
    }
    setLoading(true)
    setError('')
    try {
      ...
      const { data: photoData, error: photoError } = await supabase.storage
        .from('media').upload(photoPath, photo)
      if (photoError) throw photoError

      const videoPath = `videos/${Date.now()}-${video.name}`
      const { data: videoData, error: videoError } = await supabase.storage
        .from('media').upload(videoPath, video)
      ...
      const { error: dbError } = await supabase.from('submissions').insert({
        word, photo_url: photoUrl, video_url: videoUrl, status: 'pending'
      })
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      <header className="bg-blue-700 text-white py-4 px-8 flex items-center justify-between">
        ...
        <button onClick={logout} className="text-sm bg-blue-500 px-4 py-2 rounded-lg hover:bg-blue-400 transition">
          Çıkış
        </button>
      </header>
